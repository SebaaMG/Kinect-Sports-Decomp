typedef unsigned char undefined1;
typedef unsigned char byte;
typedef unsigned char undefined;
typedef unsigned char bool;
#define true 1
#define false 0
typedef unsigned short undefined2;
typedef unsigned short ushort;
typedef unsigned short word;
typedef unsigned int undefined4;
typedef unsigned int uint;
typedef unsigned int dword;
typedef unsigned int ulong;
typedef unsigned __int64 undefined8;
typedef unsigned __int64 ulonglong;
typedef unsigned __int64 qword;
typedef __int64 longlong;
typedef int (*code)();
typedef unsigned char U8;
typedef unsigned short U16;
typedef unsigned int U32;
typedef unsigned __int64 U64;
typedef signed char S8;
typedef signed short S16;
typedef signed int S32;
typedef __int64 S64;
typedef struct { U64 lo, hi; } V16;


ulonglong fn_82F48BA8(int param_1,uint param_2,int param_3,uint *param_4)

{
  int *piVar1;
  uint uVar2;
  ulonglong uVar3;
  undefined4 *puVar4;
  uint uVar5;
  
  if ((param_3 == 0) || ((param_2 != 1 && (param_4 == (uint *)0x0)))) {
    uVar3 = 0xffffffff80004003;
  }
  else if (((*(int *)(param_1 + 4) == 0) || (*(int *)(param_1 + 8) == 0)) ||
          (*(int *)(param_1 + 0xc) == 0)) {
    uVar3 = 0xffffffff80004005;
  }
  else {
    uVar2 = *(int *)(param_1 + 8) - *(int *)(param_1 + 0xc) >> 2;
    uVar5 = param_2;
    if (uVar2 < param_2) {
      uVar5 = uVar2;
    }
    uVar3 = (ulonglong)(uVar2 < param_2);
    if (param_4 != (uint *)0x0) {
      *param_4 = uVar5;
    }
    if (uVar5 != 0) {
      puVar4 = (undefined4 *)(param_3 + -4);
      do {
        uVar5 = uVar5 - 1;
        piVar1 = (int *)**(undefined4 **)(param_1 + 0xc);
        puVar4 = puVar4 + 1;
        *puVar4 = piVar1;
        (**(code **)(*piVar1 + 4))();
        *(int *)(param_1 + 0xc) = *(int *)(param_1 + 0xc) + 4;
      } while (uVar5 != 0);
    }
  }
  return uVar3;
}

