typedef unsigned char undefined1;
typedef unsigned char byte;
typedef unsigned char undefined;
typedef unsigned char bool;
#define true 1
#define false 0
typedef unsigned short undefined2;
typedef unsigned short ushort;
typedef unsigned short word;
typedef unsigned int undefined4;
typedef unsigned int uint;
typedef unsigned int dword;
typedef unsigned int ulong;
typedef unsigned __int64 undefined8;
typedef unsigned __int64 ulonglong;
typedef unsigned __int64 qword;
typedef __int64 longlong;
typedef int (*code)();
typedef unsigned char U8;
typedef unsigned short U16;
typedef unsigned int U32;
typedef unsigned __int64 U64;
typedef signed char S8;
typedef signed short S16;
typedef signed int S32;
typedef __int64 S64;
typedef struct { U64 lo, hi; } V16;
#define NAN(x) ((x) != (x))
extern int fn_8234B0C8();
extern int fn_827F5708();
extern unsigned int lbl_821954D8;
extern unsigned int lbl_821CC160;


void fn_8234B338(double param_1,int param_2)

{
  undefined4 *puVar1;
  double dVar2;
  double dVar3;
  double dVar4;
  double dVar5;
  
  if (*(int *)(param_2 + 0x30) != 0) {
    dVar2 = (double)(float)((double)*(float *)(param_2 + 0x30) - param_1);
    dVar5 = (double)lbl_821CC160;
    dVar4 = -dVar2;
    dVar3 = dVar5;
    if (*(float *)(&lbl_821954D8 +
                  ((uint)(byte)((dVar4 < dVar5) << 2) | (uint)(NAN(dVar4) || NAN(dVar5)) << 2)) <
        0.0) {
      dVar3 = dVar2;
    }
    *(float *)(param_2 + 0x30) = (float)dVar3;
    if (dVar3 == dVar5) {
      puVar1 = (undefined4 *)(param_2 + 4);
      if (0xf < *(uint *)(param_2 + 0x18)) {
        puVar1 = (undefined4 *)*puVar1;
      }
      fn_8234B0C8(dVar5,param_2,puVar1);
      if ((*(int *)(param_2 + 0x34) != 0) && (*(int *)(param_2 + 0x24) != 0)) {
        fn_827F5708(dVar5);
      }
    }
  }
  return;
}

