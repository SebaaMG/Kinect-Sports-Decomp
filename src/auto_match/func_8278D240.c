typedef unsigned char undefined1;
typedef unsigned char byte;
typedef unsigned char undefined;
typedef unsigned char bool;
#define true 1
#define false 0
typedef unsigned short undefined2;
typedef unsigned short ushort;
typedef unsigned short word;
typedef unsigned int undefined4;
typedef unsigned int uint;
typedef unsigned int dword;
typedef unsigned int ulong;
typedef unsigned __int64 undefined8;
typedef unsigned __int64 ulonglong;
typedef unsigned __int64 qword;
typedef __int64 longlong;
typedef int (*code)();
typedef unsigned char U8;
typedef unsigned short U16;
typedef unsigned int U32;
typedef unsigned __int64 U64;
typedef signed char S8;
typedef signed short S16;
typedef signed int S32;
typedef __int64 S64;
typedef struct { U64 lo, hi; } V16;
extern int fn_8268ACE8();
extern int fn_8268AE58();
extern int fn_8268B610();
extern int fn_8268BEA8();
extern int fn_8278CF00();
extern int fn_82F66570();


void fn_8278D240(int param_1,char *param_2,char *param_3)

{
  char cVar1;
  char *pcVar2;
  int iVar3;
  ushort uVar4;
  
  pcVar2 = param_2;
  if (param_3 == (char *)0xffffffff) {
    do {
      cVar1 = *pcVar2;
      pcVar2 = pcVar2 + 1;
    } while (cVar1 != '\0');
    param_3 = pcVar2 + (-1 - (int)param_2);
  }
  pcVar2 = param_2;
  if (param_3 == (char *)0xffffffff) {
    do {
      cVar1 = *pcVar2;
      pcVar2 = pcVar2 + 1;
    } while (cVar1 != '\0');
    param_3 = pcVar2 + (-1 - (int)param_2);
  }
  if ((*(ushort *)(param_1 + 0x2a) >> 0xb & 1) != 0) {
    pcVar2 = (char *)fn_8268ACE8((uint *)(param_1 + 8));
    if ((pcVar2 != param_3) ||
       (iVar3 = fn_8268AE58(((ulonglong)*(uint *)(param_1 + 8) & 0xfffffffc) + 8,param_2,param_3
                               ), iVar3 != 0)) {
      fn_8278CF00(param_1);
    }
  }
  fn_8268BEA8(param_1 + 8);
  fn_8268B610(param_1 + 8,param_2,param_3);
  *(ushort *)(param_1 + 0x2a) = *(ushort *)(param_1 + 0x2a) | 4;
  iVar3 = fn_82F66570(((ulonglong)*(uint *)(param_1 + 8) & 0xfffffffc) + 8,0x2c);
  if (iVar3 == 0) {
    uVar4 = *(ushort *)(param_1 + 0x2a) | 0x1000;
  }
  else {
    uVar4 = *(ushort *)(param_1 + 0x2a) & 0xefff;
  }
  *(ushort *)(param_1 + 0x2a) = uVar4;
  return;
}

