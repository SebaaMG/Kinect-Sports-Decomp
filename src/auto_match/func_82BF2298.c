typedef unsigned char undefined1;
typedef unsigned char byte;
typedef unsigned char undefined;
typedef unsigned char bool;
#define true 1
#define false 0
typedef unsigned short undefined2;
typedef unsigned short ushort;
typedef unsigned short word;
typedef unsigned int undefined4;
typedef unsigned int uint;
typedef unsigned int dword;
typedef unsigned int ulong;
typedef unsigned __int64 undefined8;
typedef unsigned __int64 ulonglong;
typedef unsigned __int64 qword;
typedef __int64 longlong;
typedef int (*code)();
typedef unsigned char U8;
typedef unsigned short U16;
typedef unsigned int U32;
typedef unsigned __int64 U64;
typedef signed char S8;
typedef signed short S16;
typedef signed int S32;
typedef __int64 S64;
typedef struct { U64 lo, hi; } V16;
extern int fn_82BE6268();
extern int fn_82BE6BD8();
extern int fn_82F67FE8();
extern int fn_82F691F0();


undefined8 fn_82BF2298(int param_1,int param_2,int param_3)

{
  char cVar1;
  int iVar2;
  int *piVar3;
  int iVar4;
  
  iVar4 = 0;
  if (0 < param_3) {
    piVar3 = (int *)(param_1 + 0x38);
    do {
      iVar2 = *piVar3;
      if ((iVar2 == 0) || (*(int *)(param_1 + 0x3c) == iVar2)) {
        iVar2 = fn_82F67FE8((char *)(iVar4 + param_2),0xffffffff820eb6bc,piVar3);
        if (iVar2 == -1) {
          return 0;
        }
        cVar1 = *(char *)(iVar4 + param_2);
        while ((cVar1 != '\n' && (iVar4 < param_3))) {
          iVar4 = iVar4 + 1;
          cVar1 = *(char *)(iVar4 + param_2);
        }
        iVar4 = iVar4 + 1;
        if (param_3 < iVar4) {
          return 0;
        }
        *(undefined4 *)(param_1 + 0x3c) = 0;
        *(int *)(param_1 + 0x40) = *piVar3 + *(int *)(param_1 + 0x40);
        if (*piVar3 == 0) {
          fn_82BE6BD8(*(undefined4 *)(param_1 + 4));
          *(undefined4 *)(param_1 + 0x30) = 0;
          *(undefined4 *)(param_1 + 8) = 0;
                    /* WARNING: Subroutine does not return */
          fn_82F691F0(*(undefined4 *)(param_1 + 0x20),0,*(undefined4 *)(param_1 + 0x2c));
        }
      }
      else {
        iVar2 = iVar2 - *(int *)(param_1 + 0x3c);
        if (param_3 - iVar4 < iVar2) {
          iVar2 = param_3 - iVar4;
        }
        fn_82BE6268(*(undefined4 *)(param_1 + 4),iVar4 + param_2,iVar2);
        iVar4 = iVar2 + iVar4;
        iVar2 = *(int *)(param_1 + 0x3c) + iVar2;
        *(int *)(param_1 + 0x3c) = iVar2;
        if ((iVar2 == *piVar3) && (iVar4 = iVar4 + 2, param_3 < iVar4)) {
          return 0;
        }
      }
    } while (iVar4 < param_3);
  }
  return 0;
}

