typedef unsigned char undefined1;
typedef unsigned char byte;
typedef unsigned char undefined;
typedef unsigned char bool;
#define true 1
#define false 0
typedef unsigned short undefined2;
typedef unsigned short ushort;
typedef unsigned short word;
typedef unsigned int undefined4;
typedef unsigned int uint;
typedef unsigned int dword;
typedef unsigned int ulong;
typedef unsigned __int64 undefined8;
typedef unsigned __int64 ulonglong;
typedef unsigned __int64 qword;
typedef __int64 longlong;
typedef int (*code)();
typedef unsigned char U8;
typedef unsigned short U16;
typedef unsigned int U32;
typedef unsigned __int64 U64;
typedef signed char S8;
typedef signed short S16;
typedef signed int S32;
typedef __int64 S64;
typedef struct { U64 lo, hi; } V16;
extern int fn_82CE5410();
extern int fn_82CED5B8();
extern unsigned int lbl_8212FC60;
extern unsigned int lbl_82138924;


void fn_82D4F308(undefined4 *param_1)

{
  int *piVar1;
  int iVar2;
  ulonglong uVar3;
  longlong lVar4;
  
  *param_1 = &lbl_82138924;
  iVar2 = fn_82CE5410();
  piVar1 = *(int **)(iVar2 + 0x10);
  param_1[0x18] = 0;
  if ((param_1[0x19] & 0x80000000) == 0) {
    (**(code **)(*piVar1 + 0x10))(piVar1,param_1[0x17],param_1[0x19] & 0x3fffffff,2);
  }
  param_1[0x17] = 0;
  param_1[0x19] = 0x80000000;
  iVar2 = fn_82CE5410();
  uVar3 = (ulonglong)(uint)param_1[0x15];
  piVar1 = *(int **)(iVar2 + 0x10);
  if (0 < (int)param_1[0x15]) {
    lVar4 = (ulonglong)(uint)param_1[0x14] + 4;
    do {
      fn_82CED5B8(lVar4);
      uVar3 = uVar3 - 1;
      lVar4 = lVar4 + 8;
    } while (uVar3 != 0);
  }
  param_1[0x15] = 0;
  if ((param_1[0x16] & 0x80000000) == 0) {
    (**(code **)(*piVar1 + 0x10))(piVar1,param_1[0x14],param_1[0x16] & 0x3fffffff,8);
  }
  param_1[0x14] = 0;
  param_1[0x16] = 0x80000000;
  iVar2 = fn_82CE5410();
  piVar1 = *(int **)(iVar2 + 0x10);
  param_1[0x12] = 0;
  if ((param_1[0x13] & 0x80000000) == 0) {
    (**(code **)(*piVar1 + 0x10))(piVar1,param_1[0x11],param_1[0x13] & 0x3fffffff,0xc);
  }
  param_1[0x11] = 0;
  param_1[0x13] = 0x80000000;
  iVar2 = fn_82CE5410();
  piVar1 = *(int **)(iVar2 + 0x10);
  param_1[0xf] = 0;
  if ((param_1[0x10] & 0x80000000) == 0) {
    (**(code **)(*piVar1 + 0x10))(piVar1,param_1[0xe],param_1[0x10] & 0x3fffffff,1);
  }
  param_1[0xe] = 0;
  param_1[0x10] = 0x80000000;
  iVar2 = fn_82CE5410();
  piVar1 = *(int **)(iVar2 + 0x10);
  param_1[0xc] = 0;
  if ((param_1[0xd] & 0x80000000) == 0) {
    (**(code **)(*piVar1 + 0x10))(piVar1,param_1[0xb],param_1[0xd] & 0x3fffffff,4);
  }
  param_1[0xb] = 0;
  param_1[0xd] = 0x80000000;
  iVar2 = fn_82CE5410();
  piVar1 = *(int **)(iVar2 + 0x10);
  param_1[9] = 0;
  if ((param_1[10] & 0x80000000) == 0) {
    (**(code **)(*piVar1 + 0x10))(piVar1,param_1[8],param_1[10] & 0x3fffffff,2);
  }
  param_1[8] = 0;
  param_1[10] = 0x80000000;
  iVar2 = fn_82CE5410();
  piVar1 = *(int **)(iVar2 + 0x10);
  param_1[6] = 0;
  if ((param_1[7] & 0x80000000) == 0) {
    (**(code **)(*piVar1 + 0x10))(piVar1,param_1[5],param_1[7] & 0x3fffffff,1);
  }
  param_1[5] = 0;
  param_1[7] = 0x80000000;
  iVar2 = fn_82CE5410();
  piVar1 = *(int **)(iVar2 + 0x10);
  param_1[3] = 0;
  if ((param_1[4] & 0x80000000) == 0) {
    (**(code **)(*piVar1 + 0x10))(piVar1,param_1[2],param_1[4] & 0x3fffffff,0x10);
  }
  param_1[2] = 0;
  param_1[4] = 0x80000000;
  *param_1 = &lbl_8212FC60;
  return;
}

