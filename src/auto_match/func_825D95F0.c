typedef unsigned char undefined1;
typedef unsigned char byte;
typedef unsigned char undefined;
typedef unsigned char bool;
#define true 1
#define false 0
typedef unsigned short undefined2;
typedef unsigned short ushort;
typedef unsigned short word;
typedef unsigned int undefined4;
typedef unsigned int uint;
typedef unsigned int dword;
typedef unsigned int ulong;
typedef unsigned __int64 undefined8;
typedef unsigned __int64 ulonglong;
typedef unsigned __int64 qword;
typedef __int64 longlong;
typedef int (*code)();
typedef unsigned char U8;
typedef unsigned short U16;
typedef unsigned int U32;
typedef unsigned __int64 U64;
typedef signed char S8;
typedef signed short S16;
typedef signed int S32;
typedef __int64 S64;
typedef struct { U64 lo, hi; } V16;
extern int fn_82522D98();
extern int fn_8262F8D8();
extern int fn_8262FEC8();
extern int fn_82630040();


void fn_825D95F0(int param_1)

{
  uint uVar1;
  int iVar2;
  longlong lVar3;
  
  if (*(char *)(param_1 + 0x1c0) != '\0') {
    uVar1 = *(uint *)(param_1 + 0x20);
    if (uVar1 == 0) {
      lVar3 = (ulonglong)*(uint *)(param_1 + 0x28) * 0x28;
    }
    else {
      if ((uVar1 != 1) && (2 < uVar1)) {
        if (uVar1 != 3) {
          return;
        }
        lVar3 = fn_82630040((ulonglong)*(uint *)(param_1 + 0x28) * 0x28,0,0);
        *(int *)(param_1 + 0x134) = (int)lVar3;
        if ((longlong)-(ulonglong)(lVar3 == 0) < 0) {
          return;
        }
        *(undefined4 *)(param_1 + 0x13c) = 0;
        iVar2 = *(int *)(param_1 + 0x134);
        if (*(int *)(param_1 + 0x13c) == 0) {
          fn_8262FEC8();
          *(undefined4 *)(param_1 + 0x134) = 0;
          return;
        }
        uVar1 = *(uint *)(iVar2 + 0x18) & 0xfffffffc;
                    /* WARNING: Subroutine does not return */
        fn_8262F8D8(iVar2,10,0,uVar1,0,uVar1,*(uint *)(iVar2 + 0x1c) & 0x3fffffc,0);
      }
      lVar3 = ((ulonglong)*(uint *)(param_1 + 0x28) & 0x7ffffff) << 5;
    }
    lVar3 = fn_82630040(lVar3,0,0);
    *(int *)(param_1 + 0x134) = (int)lVar3;
    if (-1 < (longlong)-(ulonglong)(lVar3 == 0)) {
                    /* WARNING: Subroutine does not return */
      fn_82522D98((ulonglong)*(uint *)(param_1 + 0x34) * 0x58);
    }
  }
  return;
}

