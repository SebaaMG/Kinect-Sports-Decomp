typedef unsigned char undefined1;
typedef unsigned char byte;
typedef unsigned char undefined;
typedef unsigned char bool;
#define true 1
#define false 0
typedef unsigned short undefined2;
typedef unsigned short ushort;
typedef unsigned short word;
typedef unsigned int undefined4;
typedef unsigned int uint;
typedef unsigned int dword;
typedef unsigned int ulong;
typedef unsigned __int64 undefined8;
typedef unsigned __int64 ulonglong;
typedef unsigned __int64 qword;
typedef __int64 longlong;
typedef int (*code)();
typedef unsigned char U8;
typedef unsigned short U16;
typedef unsigned int U32;
typedef unsigned __int64 U64;
typedef signed char S8;
typedef signed short S16;
typedef signed int S32;
typedef __int64 S64;
typedef struct { U64 lo, hi; } V16;
extern unsigned int *auStack_2c;
extern int fn_82468470();
extern int fn_8259BDA8();
extern int fn_82865170();
extern int fn_82866400();


undefined8 fn_82871CD0(int param_1)

{
  undefined4 *puVar1;
  int *piVar2;
  undefined4 *puVar3;
  char cVar5;
  int iVar4;
  undefined4 *puStack_30;
  undefined1 auStack_2c [44];
  
  puVar1 = *(undefined4 **)(param_1 + 0x28);
  puStack_30 = (undefined4 *)*puVar1;
  while (puVar3 = puStack_30, puStack_30 != puVar1) {
    piVar2 = (int *)puStack_30[3];
    (**(code **)(*piVar2 + 0x40))(piVar2);
    cVar5 = (**(code **)*piVar2)(piVar2);
    if (cVar5 == '\0') {
      fn_8259BDA8(&puStack_30);
      fn_82468470(auStack_2c,param_1 + 0x24,puVar3);
      fn_82865170();
      iVar4 = fn_82866400();
      *(int *)(iVar4 + 0x10) = *(int *)(iVar4 + 0x10) + -1;
      (**(code **)(*piVar2 + 0x70))(piVar2,1);
    }
    else {
      fn_8259BDA8(&puStack_30);
    }
    puVar1 = *(undefined4 **)(param_1 + 0x28);
  }
  return 0x20110000;
}

